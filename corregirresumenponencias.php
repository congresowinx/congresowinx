...
<html lang="es">
    <head>
        ...
        <title>Corrección de resumen ponencia</title>
       <meta charset="UTF-8">  
...
<div class="ContenedorPrincipal"> 
 ...
 <?php
  ...
  ?>
        <p class="Tema">Registro de corrección resumen para ponencia</p>
...
<table>
                       ...
                       <br>  <p class="temaSec">1.- Usted deberá corregir lo siguiente:.</p>   
                        <center>    <tr>
                                <td class="C1">

                                    <label for="Corrección">Corrección:</label>
                                </td>

                <?php 
                   $idt = '123125200'; //$_GET
                $conexion = pg_connect("host=localhost dbname=congresowinx user=congresowinx password=W1nxC0ngr3s032511");
                  $queryob=("Select * from evaluacion_trabajos where trabajos_id='$idt' ");
                            $connob=pg_query($conexion,$queryob);
                         $cantidad= pg_num_rows($connob);
                                 if ($cantidad>0){}
                                                     if(!$connob){
                                                        die(pg_error($conexion));
                                                         }

                                               if (pg_num_rows($connob) > 0) {
                                                    while($rowData = pg_fetch_array($connob)){
                                                      $idev=$rowData["evaluacion_id"];
                                                     
                      $queryob2=("Select * from evaluacion_resumen where evaluacion_id='$idev' ");
                            $connob2=pg_query($conexion,$queryob2);
                         $cantidad= pg_num_rows($connob2);
                                 if ($cantidad>0){}
                                                     if(!$connob2){
                                                        die(pg_error($conexion));
                                                         }

                                               if (pg_num_rows($connob2) > 0) {
                                                    while($rowDataob2 = pg_fetch_array($connob2)){
                                                      $obse=$rowDataob2["observaciones"];
                ...
                ?>
                                ...
                                <td class="C2">
                                    <textarea class="textarea2" type="text"  name="correccion" value="<?php echo $obse ?>"readonly><?php echo $obse ?></textarea>
                        ...
                        <center>    <tr>
                                <td class="C1">
                                   ...
                                   <br><br> <br> <label for="Title">Título del resumen:</label>
                                </td>
                                <td class="C2">
                                    <br><br><textarea class="textarea2" type="text"  name="titulo" placeholder="" style="text-transform:uppercase;"required></textarea>
                        ...
                        <table>
                        <br><br><p class="temaSec">2.- El resumen del trabajo debe de contener no más de 300 palabras.</p>  
                         	<center>    <tr>
                                 <td class="C1">
     ...
     //SACAREMOS EL ID DE LA TABLA REFERENCIA:
      ...
      do {
          ...
          $idreferencia--;
                 ...
                 if($redir==0){             
                        echo "<script>alert('Corrección exitosa y enviada.');window.location.replace('https://laboratoriosistemas.cuautitlan2.unam.mx/congresowinx/WinxCongreso/menu.php');</script>";  }
